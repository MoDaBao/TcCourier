...
@property (nonatomic, strong) ShoppingInfoView *shopView;
...
//    [_mapView setCenterCoordinate:CLLocationCoordinate2DMake(self.latitude.floatValue, self.longitude.floatValue)];
    ...
    AMapNaviPoint *endPoint =[AMapNaviPoint locationWithLatitude:self.storeInfoModel.latitudeG.floatValue longitude:self.storeInfoModel.longitudeG.floatValue];
    
    
    [self.driveManager calculateDriveRouteWithStartPoints:@[startPoint] endPoints:@[endPoint] wayPoints:nil drivingStrategy:AMapNaviDrivingStrategySingleAvoidCongestion];//单路径: 躲避拥堵
...
- (void)driveManagerOnCalculateRouteSuccess:(AMapNaviDriveManager *)driveManager {
    NSLog(@"路径规划成功");
    self.shopView.disL.text = [NSString stringWithFormat:@"商家距离:%ld米",driveManager.naviRoute.routeLength];
